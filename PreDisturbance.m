...
BC = Benthic_Change; 
BC.Sediment = BC.Sediment*1000; %Changes to kg/ha
...
%%%%%%%PLOTTING FIGURE 2D and EXTENDED DATA FIGURE 3%%%%%%%%%%%%%%%%%%
...
opts.VariableTypes = ["double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "categorical", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double"];
...
% Specify variable properties
opts = setvaropts(opts, "Trajectory", "EmptyFieldRule", "auto");

% Import the data
TRAJECTORY = readtable("/Users/jgove/Documents/jamisongove/EOD/KONA_IEA/PaperI/data/Analysis/Final_Data/Revision/Submission/CORAL.REEF.TRAJECTORY.csv", opts);
...
ind_pos = TRAJECTORY.Coral_Change >3; %Get positive trajectory indices
ind_neg = TRAJECTORY.Coral_Change <-3;% Get negative trajectory indices
headers = TRAJECTORY.Properties.VariableNames; %Get column headers 

%Subset columns that we want to show - this represents all factors. 
drivers2avg = {'total_biomass','herbivores','scrapers','browsers','grazers',...
    'HPop15km','WPow975pct','GearRegRank','Depth','SST_MEAN','SST_STD','CHL_CLIM_M',...
    'Sediment','RainAnnSum','RainMax3dS','Nuts','Imperv','OSDS_Eff'};
  
JK = ones(numel(drivers2avg),3)*NaN; %set variable to populate and make NaN
%For loop to calculate the delta in jackknife means between positive
%trajectory reefs and negative trajectory reefs. 
for i = 1:numel(drivers2avg)
    col = matches(headers,drivers2avg(i)); %find the column of data
    YY = jackknife(@nanmean,TRAJECTORY{ind_pos,col}); %Jackknife all positive trajectory values
    NN = jackknife(@nanmean,TRAJECTORY{ind_neg,col});% Jackknife all negative trajectory values
    %Assign variables as follows, with Delta as Positive Trajectory - Negative Trajectory
    Delta = mean(YY)-mean(NN); %mean delta, min delta, max delta
    DeltaMin = min(YY)-max(NN);
    DeltaMax = max(YY) - min(NN);
JK(i,:) = [Delta; DeltaMin; DeltaMax];%Assign in variables
end
%Changing individual factors so as to align on the same graph. 
%Multiple fish by 10 to go from g/m2 to kg/ha
JK(1:5,:) = JK(1:5,:).*10; 
%Change Human Popuation to 10^3 
col = matches(drivers2avg,'HPop15km');
JK(col,:) = JK(col,:)./1000;
%Change Wastewater to 10^3 
col = matches(drivers2avg,'OSDS_Eff');
JK(col,:) = JK(col,:)./1000;
%Change Urban Runoff  to 10^2 
col = matches(drivers2avg,'Imperv');
JK(col,:) = JK(col,:)./100;
%Change Sediment  to 10^2 
col = matches(drivers2avg,'Sediment');
JK(col,:) = JK(col,:)./100;
%Change Annual Rain to 10^3 
col = matches(drivers2avg,'RainAnnSum');
JK(col,:) = JK(col,:)./1000;
%Change Peak Rain to 10^3 
col = matches(drivers2avg,'RainMax3dS');
JK(col,:) = JK(col,:)./1000;

%NEXT, sort to ensure they are in pos to neg order, from left to right
[A I] = sort(JK(:,1),'descend'); JK(:,:) = JK(I,:); 

%Sort the drivers2avg and rename to drivers. This is the order to plot
drivers = drivers2avg(I); 

figure('Renderer', 'painters', 'units','centimeters','Position', [5 50 14.1 10])
  graycolor = [0.4 0.4 0.4]%Setting a color...not required; 
for i = 1:numel(drivers)
    e = errorbar(i,JK(i,1),[JK(i,1)-JK(i,2)],[JK(i,3)-JK(i,1)],'vertical','.','Markersize',20,'MarkerEdgeColor',graycolor);        
        e.LineWidth = 3;
        e.Color = graycolor
        e.CapSize = 0;
        hold on
...
%cleaning up figure
ylim([-70 400]); 
set(gca,'xtick',[1:1:18]); set(gca,'ytick',[-60:20:400]);
set(gca,'xticklabel',drivers); %sets labels of actual plotted variables
xlim([0.5 18.5])
yline(0,':k')
ylabel('Mean Delta')

%REPLOT into 2 panels similar to publication. They are split in order to
%best visualize their differences all on the same y axis. 
%LEFT PANEL
drivers2plot1= {'total_biomass','herbivores','scrapers','browsers','grazers',...
    'HPop15km','WPow975pct','Nuts','Imperv','OSDS_Eff'};

figure('Renderer', 'painters', 'units','centimeters','Position', [5 50 14.1 10]); hold on
for i = 1:numel(drivers2plot1)
  graycolor = [0.4 0.4 0.4]
  col = matches(drivers,drivers2plot1(i));
    e = errorbar(i,JK(col,1),[JK(col,1)-JK(col,2)],[JK(col,3)-JK(col,1)],'vertical','.','Markersize',20,'MarkerEdgeColor',graycolor);        
        e.LineWidth = 3;
        e.Color = graycolor
        e.CapSize = 0;
        hold on
...
ylim([-70 400]);
set(gca,'xtick',[1:1:numel(drivers2plot1)]); set(gca,'ytick',[-80:20:400]);
set(gca,'xticklabel',drivers2plot1); %Confirming it's in the correct order before renaming below
set(gca,'xticklabel',{'Total biomass','Herbivores','Scrapers','Browsers','Grazers','Humans','Waves','Nutrients','Urban Runoff','Wastewater'});
xlim([0.5 numel(drivers2plot1)+0.5])
yline(0,':k')
ylabel('Mean Delta')

%RIGHT PANEL
drivers2plot2= {'GearRegRank','Depth','Sediment','SST_MEAN','CHL_CLIM_M','SST_STD',...
  'RainMax3dS','RainAnnSum'};
%This is for the axis break 

figure('Renderer', 'painters', 'units','centimeters','Position', [5 50 14.1 10]); hold on
for i = 1:numel(drivers2plot2)
  graycolor = [0.4 0.4 0.4]
  col = matches(drivers,drivers2plot2(i));
    e = errorbar(i,JK(col,1),[JK(col,1)-JK(col,2)],[JK(col,3)-JK(col,1)],'vertical','.','Markersize',20,'MarkerEdgeColor',graycolor);        
        e.LineWidth = 3;
        e.Color = graycolor
        e.CapSize = 0;
...
ylim([-15 20]);
set(gca,'xtick',[1:1:numel(drivers2plot2)]); set(gca,'ytick',[-15:5:20]);
set(gca,'xticklabel',drivers2plot2); %Confirming it's in the correct order before renaming below
set(gca,'xticklabel',{'Fishing Gear','Depth','Sediment Input','Mean SST','Phytoplankton','SST Var','Peak Rainfall','Annual Rainfall'});
xlim([0.5 numel(drivers2plot2)+0.5])
yline(0,':k')
ylabel('Mean Delta')
...
%Note that figure refining was all done in Adobe Illustrator 
%clearvars -except BC Benthic_Change drivers TRAJECTORY
%% CORRELATON PLOT
BCplot = BC; 
%Get rid of values that are not positive or negative
ind = matrix2avg.Coral_Change <=3 & matrix2avg.Coral_Change >= -3;
BCplot(ind,:) = []; 
%Now only want variables that we plot in Fig 2d.
BCplot = removevars(BCplot, {'SiteID','GridID','Lat','Long','Year_Start','Year_End','Years',...
    'CoralStart','CoralEnd','Coral_Change','Turf_Change','Macro_Change','CCA_Change',...
    'OSDS_N','Golf_N'});
